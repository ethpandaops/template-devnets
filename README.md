...
The state key in `main.tf` (`infrastructure/template-devnet-0/terraform.tfstate`) lives in the `merge-testnets` bucket, which every devnet repo shares. When forking this template, rename it to `infrastructure/<name>-devnet-N/terraform.tfstate` along with the other `template` placeholders (`devnet_name`, `ethereum_network`, `template_hcloud_token`, `prefix` in `scripts/run.zsh`). Backend blocks cannot read variables, so this one has to be edited by hand.
...
1. Open the [nodes.tf](terraform/devnet-0/nodes.tf) file located in the terraform/devnet-0/ directory.

2. This file contains variables for each different nodes, containing node count, size, and validator ranges.

3. Adjust the validator indexes in the terraform/devnet-0/nodes.tf file based on your desired allocation of validators to nodes.

For example, let's say you want to assign validator index 0-24 to a single lodestar-besu node and validator index 25-224 to two lighthouse-nethermind nodes. Update the nodes.tf file as follows:
```terraform
